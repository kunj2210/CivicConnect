    ...
    return (
        <div className="space-y-8 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                <StatCard title="Total Issues" value={mockStats[0].value} color="blue" icon={TrendingUp} trend="+12.5" />
                <StatCard title="Resolved" value={mockStats[1].value} color="green" icon={CheckCircle} trend="+8.2" />
                <StatCard title="In Progress" value={mockStats[2].value} color="yellow" icon={Clock} trend="-2.4" />
                <StatCard title="Pending" value={mockStats[3].value} color="red" icon={AlertCircle} trend="+5.1" />
            </div>
                ...
                <div className={`p-6 backdrop-blur-xl rounded-2xl shadow-lg border ${darkMode ? 'bg-gray-800/80 border-gray-700' : 'bg-white/80 border-white/20'}`}>
                    ...
                    <div className="h-64 flex justify-center relative">
                        ...
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[60%] text-center pointer-events-none">
                            <span className={`text-3xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>125</span>
            ...
            <div className={`p-6 backdrop-blur-xl rounded-2xl shadow-lg border ${darkMode ? 'bg-gray-800/80 border-gray-700' : 'bg-white/80 border-white/20'}`}>
                ...
                <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className={`flex items-center justify-between p-4 rounded-xl transition-colors border cursor-pointer ${darkMode ? 'bg-gray-700/50 hover:bg-blue-900/20 border-transparent hover:border-blue-800' : 'bg-gray-50/50 hover:bg-blue-50/50 border-transparent hover:border-blue-100'}`}>
                            <div className="flex items-center">
                                ...
                                <div>
                                    <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>New Issue Reported</p>
                                    <p className="text-sm text-gray-500">Pothole near MG Road reported by Citizen</p>
                                </div>
                            </div>
                            <span className={`text-xs font-medium px-2 py-1 rounded-md shadow-sm ${darkMode ? 'text-gray-300 bg-gray-600' : 'text-gray-400 bg-white'}`}>2h ago</span>
                        </div>
                    ))}
                </div>
            </div>
