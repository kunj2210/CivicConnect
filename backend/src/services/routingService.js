...
export class RoutingService {
    ...
    static async identifyJurisdiction(latitude, longitude) {
        try {
            ...
            let [result] = await sequelize.query('SELECT name FROM ulb_boundaries WHERE ST_Contains(geom, ST_SetSRID(ST_Point(:lon, :lat), 4326)) LIMIT 1', { replacements: { lat: latitude, lon: longitude }, type: QueryTypes.SELECT });
            if (result && result.name) {
                return result.name;
            }
            // Fallback using ST_Intersects for edge cases
            [result] = await sequelize.query('SELECT name FROM ulb_boundaries WHERE ST_Intersects(geom, ST_SetSRID(ST_Point(:lon, :lat), 4326)) LIMIT 1', { replacements: { lat: latitude, lon: longitude }, type: QueryTypes.SELECT });
            if (result && result.name) {
                return result.name;
            }
